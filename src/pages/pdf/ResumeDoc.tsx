            ...
            <Document>
                <Page size="A4">
                    <View style={styles.maincontainer}>
                        ...
                        <Text style={styles.status}>{data?.info?.objective || "Enter objective"}</Text>
                    </View>
                    <View style={styles.infoContainer}>
                        <Text style={styles.infoText}>{data?.email}</Text>
                        <Text style={styles.infoText}>{data?.phoneNo}</Text>
                        <Text style={styles.infoText}>{data?.info?.address || "Enter address"}</Text>
                    </View>
                    {/* exprience */}
                    {
                                            ...
                                            <>
                                                <View style={styles.exprienceContainer} key={index}>
                                                    ...
                                                    <View style={styles.companyInfo}>
                                                        <Text style={styles.years}>{convertDate(item?.startDate)} - {(item?.endDate) ? convertDate(item?.endDate) : "Continue"}</Text>
                                                    </View>
                                                    <Text style={styles.workDescription}>{item?.jobProfile}</Text>
                                                    <View style={styles.skills}>
                                                        {
                                                            item?.skills?.map((skill, i) => (
                                                                <>
                                                                    <Text style={styles.skill} key={i}>{skill}</Text>
                                                                </>
                                                            ))
                                                        }
                                        ...
                                        data?.project?.map((item: Project, index: number) => (
                                            <>
                                                <View style={styles.exprienceContainer} key={index}>
                                                    ...
                                                    <View style={styles.companyInfo}>
                                                        <Text style={styles.years}>{convertDate(item.startDate)} - {(item?.endDate) ? convertDate(item?.endDate) : "Continue"}</Text>
                                                        <Text style={styles.companyAddress}>{item?.associate}</Text>
                                                    </View>
                                ...
                                <View style={styles.education}>
                                    ...
                                    </View>
                                    {
                                        data?.education?.map((item: Education, index: number) => (
                                            <>
                                                <View style={styles.exprienceContainer} key={index}>
                                                    <Text style={styles.role}>{item?.degree} ({item?.course})</Text>
                                                    <Text style={styles.companyName}>{item?.college}</Text>
                                                    <Text style={styles.years}>{item?.admissionYear} - {item?.passYear}</Text>
